...
import { User, Shield, Key, AlertTriangle, Building, Globe, Upload, Eye, EyeOff } from "lucide-react";
...
export default function SettingsPage() {
      ...
      try {
        ...
        if (res.data.status === "ok" && res.data.data) {
          ...
          setProfile({
            ...
            latitude: d.location?.coordinates?.[1] !== undefined ? String(d.location.coordinates[1]) : "",
            longitude: d.location?.coordinates?.[0] !== undefined ? String(d.location.coordinates[0]) : "",
            facebook: d.facebook || "",
          ...
          if (d.profileImage?.secure_url || d.logo?.secure_url) setLogoPreview(d.profileImage?.secure_url || d.logo?.secure_url);
          if (d.partnerImage?.secure_url) setPartnerImagePreview(d.partnerImage.secure_url);
        }
      } catch (err) {
  ...
  const handleProfileSubmit = async (e) => {
    ...
    try {
      ...
      formData.append("selfIntroduction", profile.selfIntroduction.slice(0, 100));
      ...
      if (res.status === 200 || res.data?.status === "ok") {
        ...
        toast.success(t.profileUpdated || "Profile details updated successfully.");
        ...
        if (freshData?.profileImage?.secure_url || freshData?.logo?.secure_url) setLogoPreview(freshData.profileImage?.secure_url || freshData.logo?.secure_url);
        if (freshData?.partnerImage?.secure_url) setPartnerImagePreview(freshData.partnerImage.secure_url);
      } else {
        toast.error(res.data?.message || t.profileUpdateFailed || "Failed to update profile.");
      }
    } catch (err) {
      ...
      if (errorData?.data && Array.isArray(errorData.data)) {
        ...
        toast.error(errorData?.message || t.profileUpdateError || "Error updating profile details.");
      }
    } finally {
      setLoading(false);
    }
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.status === "ok") {
        toast.success(t.passwordChangedSuccess || "Password changed successfully. Please log in again.");
        setTimeout(() => {
        ...
        toast.error(res.data.message || t.passwordChangeFailed || "Failed to change password.");
      }
    } catch (err) {
      toast.error(err.response?.data?.message || t.passwordChangeError || "Error updating password.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const hasCoordinates = profile.latitude !== "" && profile.longitude !== "" && !isNaN(profile.latitude) && !isNaN(profile.longitude);
  ...
  useEffect(() => {
    if (!isLoaded) return;
    const imgUrl = logoPreview || profile?.profileImage?.secure_url || profile?.logo?.secure_url;
    generatePinIcon(imgUrl).then(icon => {
      if (icon) setMarkerIcon(icon);
    });
  }, [isLoaded, logoPreview, profile?.profileImage?.secure_url, profile?.logo?.secure_url]);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex bg-[#f5f0e8] p-1.5 rounded-2xl border border-[#e8ddd0] w-fit self-start gap-1">
        {[
          { key: "profile", label: t.profileDetailsTab || "Profile details", icon: User },
          { key: "security", label: t.changePasswordTab || "Change Password", icon: Key },
        ].map((tab) => (
          <button
            key={tab.key}
          ...
          <form onSubmit={handleProfileSubmit} className="flex flex-col gap-6">
            ...
            <div className="relative w-full h-48 rounded-2xl overflow-hidden border border-[#e8ddd0] bg-[#fcfaf7]">
              ...
              <div className="absolute inset-0 w-full h-full bg-[#f0e8d8] overflow-hidden group">
                {partnerImagePreview ? (
                  <img src={getOptimizedCloudinaryUrl(partnerImagePreview, 800, 300)} alt="Cover Banner" className="w-full h-full object-cover" />
                ) : (
                ...
                <label className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center text-white text-xs font-bold gap-1.5 transition-all duration-300 cursor-pointer">
                  <Upload className="w-4 h-4" /> {t.changeCover || "Change Cover Image"}
                  <input
                    type="file"
                    accept="image/*"
              ...
              <div className="absolute bottom-4 left-6 w-24 h-24 rounded-2xl border-4 border-white bg-white shadow-xl overflow-hidden flex items-center justify-center group z-10">
                {logoPreview ? (
                  <img src={getOptimizedCloudinaryUrl(logoPreview, 300, 300)} alt="Profile Logo" className="w-full h-full object-cover" />
                ) : (
                  <span className="text-3xl font-bold text-orange-600">{getPartnerName(profile)?.charAt(0) || "P"}</span>
                )}
                <label className="absolute inset-0 bg-black/55 opacity-0 group-hover:opacity-100 flex items-center justify-center text-white text-[10px] font-bold text-center p-1 transition-all duration-300 cursor-pointer">
                  <Upload className="w-3 h-3 mb-1 block mx-auto" /> {t.changeLogo || "Change Logo"}
                  <input
                    type="file"
                    accept="image/*"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex flex-col gap-1.5">
                <span className="text-[11px] font-bold text-[#3a2a1a]">{t.firstName || "Representative First Name"}</span>
                <input
                  type="text"
                  name="firstName"
              ...
              <div className="flex flex-col gap-1.5">
                <span className="text-[11px] font-bold text-[#3a2a1a]">{t.lastName || "Representative Last Name"}</span>
                <input
                  type="text"
                  name="lastName"
              ...
              <div className="flex flex-col gap-1.5">
                <span className="text-[11px] font-bold text-[#3a2a1a]">{t.phone || "Phone Number"}</span>
                <input
                  type="text"
                  name="phone"
                  value={profile.phone}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex flex-col gap-1.5">
                <span className="text-[11px] font-bold text-[#3a2a1a]">{t.company || "Company / Organization Name"}</span>
                <input
                  type="text"
                  name="company"
              ...
              <div className="flex flex-col gap-1.5">
                <span className="text-[11px] font-bold text-[#3a2a1a]">{t.websiteLabel || "Website URL"}</span>
                <input
                  type="url"
                  name="website"
              ...
              <div className="flex flex-col gap-1.5">
                <span className="text-[11px] font-bold text-[#3a2a1a]">Facebook URL</span>
                <input
                  type="url"
                  name="facebook"
              ...
              <div className="flex flex-col gap-1.5">
                <span className="text-[11px] font-bold text-[#3a2a1a]">Instagram URL</span>
                <input
                  type="url"
                  name="instagram"
              ...
              <div className="flex flex-col gap-1.5">
                <span className="text-[11px] font-bold text-[#3a2a1a]">Twitter URL</span>
                <input
                  type="url"
                  name="twitter"
              ...
              <div className="flex flex-col gap-1.5">
                <span className="text-[11px] font-bold text-[#3a2a1a]">LinkedIn URL</span>
                <input
                  type="url"
                  name="linkedin"
              ...
              <div className="flex flex-col gap-1.5">
                <span className="text-[11px] font-bold text-[#3a2a1a]">{t.regionLabel || "Region"}</span>
                <select
                  name="region"
                ...
                >
                  ...
                  {locations.regions.map(r => <option key={r} value={r}>{r}</option>)}
                </select>
              </div>

              <div className="flex flex-col gap-1.5">
                <span className="text-[11px] font-bold text-[#3a2a1a]">{t.departmentLabel || "Department"}</span>
                <select
                  name="department"
              ...
              <div className="flex flex-col gap-1.5 md:col-span-2">
                <span className="text-[11px] font-bold text-[#3a2a1a]">{t.address || "Address"}</span>
                <input
                  type="text"
                  name="address"
              ...
              <div className="flex flex-col gap-1.5">
                <span className="text-[11px] font-bold text-[#3a2a1a]">{t.cityLabel || "City"}</span>
                <input
                  type="text"
                  name="city"
                  value={profile.city}
              ...
              <div className="flex flex-col gap-1.5">
                <span className="text-[11px] font-bold text-[#3a2a1a]">{t.postalCode || "Postal Code"}</span>
                <input
                  type="text"
                  name="postalCode"
                  ...
                  onChange={(e) => {
                    ...
                    handleProfileChange({ target: { name: 'postalCode', value: val } });
                  }}
                  className="bg-[#fcfaf7] border border-[#e8ddd0] rounded-xl px-4 py-2.5 text-xs text-[#3a2a1a] outline-none focus:border-orange-600 transition-all font-medium"
                  required
                />
              ...
              <div className="flex flex-col gap-1.5">
                <span className="text-[11px] font-bold text-[#3a2a1a]">{t.latitude || "Latitude"}</span>
                <input
                  type="text"
                  step="any"
                  name="latitude"
              ...
              <div className="flex flex-col gap-1.5">
                <span className="text-[11px] font-bold text-[#3a2a1a]">{t.longitude || "Longitude"}</span>
                <input
                  type="text"
                  step="any"
                  name="longitude"
              ...
              <div className="flex flex-col gap-1.5 md:col-span-2">
                <span className="text-[11px] font-bold text-[#3a2a1a] flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-[#9a8a7a] font-normal">{t.mapInstruction || "Click on the map or drag the marker to select coordinates"}</span>
                </span>
                ...
                <div className="rounded-xl overflow-hidden border border-[#e8ddd0] h-[300px] relative z-0">
                  {loadError ? (
                    ...
                    <>
                      ...
                      <button
                        type="button"
                        onClick={(e) => { e.preventDefault(); setMapType(prev => prev === 'roadmap' ? 'hybrid' : 'roadmap'); }}
              ...
              <div className="flex flex-col gap-1.5 md:col-span-2">
                <span className="text-[11px] font-bold text-[#3a2a1a]">{t.aboutOrg || "About Organization (Description)"}</span>
                <textarea
                  ...
                  placeholder={t.aboutOrgPlaceholder || "Describe your company (max 100 characters)"}
                  maxLength="100"
                />
                <span className="text-[9px] text-[#9a8a7a] self-end">{profile.selfIntroduction.length}/100</span>
              </div>
            </div>
        ...
        {activeTab === "security" && (
          <form onSubmit={handlePasswordSubmit} className="flex flex-col gap-6 max-w-md">
            ...
            <div className="flex flex-col gap-4">
              <div className="flex flex-col gap-1.5">
                <span className="text-[11px] font-bold text-[#3a2a1a]">{t.currentPassword || "Current Password"}</span>
              ...
              <div className="flex flex-col gap-1.5">
                <span className="text-[11px] font-bold text-[#3a2a1a]">{t.newPassword || "New Password"}</span>
                <div className="relative">
                  <input
              ...
              <div className="flex flex-col gap-1.5">
                <span className="text-[11px] font-bold text-[#3a2a1a]">{t.confirmNewPassword || "Confirm New Password"}</span>
                <div className="relative">
                  <input
            ...
            <button
              ...
              className="px-6 py-2.5 bg-orange-600 hover:bg-orange-700 text-white font-bold text-xs rounded-xl self-end transition-all flex items-center justify-center shadow"
            >
              {loading ? (t.sendingLabel || "Updating...") : (t.updatePassword || "Update Password")}
            </button>
          </form>
        )}
      </div>
